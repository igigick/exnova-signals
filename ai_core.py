"""
🧠 EXNOVA AI ENGINE v4.0 — Ensemble Híbrido (Refactorizado)
NN + RandomForest + Análisis Técnico + Meta-Clasificador
Correcciones: syntax, robustez, validación, balanceo, logging
"""
...
try:
    ...
    SKLEARN_OK = False
    logger.warning("scikit-learn no disponible. Modo fallback activado.")
...
def sb_upload(data_bytes, filename, bucket=BUCKET):
    ...
    try:
        ...
        if ok:
            logger.info(f"Modelo subido: {filename}")
        else:
            logger.warning(f"Fallo al subir {filename}: {r.status_code}")
        return ok
...
def sb_download(filename, bucket=BUCKET):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return r.content
        logger.warning(f"Modelo no encontrado en nube: {filename}")
        return None
    except Exception as e:
        logger.error(f"Error descargando de Supabase: {e}")
        return None
# ═══════════════════════════════════════════════
# RED NEURONAL PROFUNDA (NumPy)
# ═══════════════════════════════════════════════
class DeepNN:
    ...
    def train(self, X, y, epochs=80, batch_size=64, class_weights=None):
        y = np.array(y)
        n = X.shape[0]
        if n == 0:
            logger.warning("Dataset vacío para entrenar NN")
            return self
        for epoch in range(epochs):
            ...
            for start in range(0, n, batch_size):
                ...
                epoch_loss += -np.sum(by * np.log(pred + 1e-8)) / bx.shape[0]
            if epoch % 20 == 0 or epoch == epochs - 1:
                logger.info(f"NN Epoch {epoch+1}/{epochs} — Loss: {epoch_loss:.4f}")
        return self
...
def candle_patterns(df):
    ...
    patterns["engulfing_bull"] = (
        (body.shift(1) < 0) & (body > 0) & (o <= c.shift(1)) & (c >= o.shift(1))
    ).astype(int)
    patterns["engulfing_bear"] = (
        (body.shift(1) > 0) & (body < 0) & (o >= c.shift(1)) & (c <= o.shift(1))
    ).astype(int)
    patterns["shooting_star"] = (
        (upper > body_abs * 2) & (lower < body_abs * 0.3) & (body < 0)
    ).astype(int)
    ...
    try:
        ...
        for col in ["Open", "High", "Low", "Close", "Volume"]:
            if col not in df.columns:
                logger.error(f"Columna {col} no encontrada en datos de {ticker}")
                return pd.DataFrame()
        return df.dropna()
    except Exception as e:
...
def technical_model(df):
    ...
    if u.engulfing_bear or u.shooting_star:
...
# ═══════════════════════════════════════════════
# META-CLASIFICADOR (con métricas por clase)
# ═══════════════════════════════════════════════
class MetaClassifier:
    ...
    def update_weights(self):
        ...
        if total > 0:
            self.weights = {k: (0.6 * self.f1[k] + 0.4 * self.accuracy[k]) / total
                           for k in ["nn", "rf", "tech"]}
...
class ExnovaAI:
    ...
    def _evaluate(self, X_test, y_test):
        ...
        if self.rf is not None and SKLEARN_OK and len(X_test) > 0:
            pred_labels = self.rf.predict(X_test)
            true_labels = np.argmax(y_test, axis=1)
            acc = np.mean(pred_labels == true_labels)
    ...
    def train(self, asset, tf, df, f):
        try:
            ...
            if n < 40:
                ...
                logger.warning(f"Muestras insuficientes para {asset}/{tf}: {n}")
                return False
            ...
            nn.train(X_train, y_train, epochs=60, batch_size=min(64, n), class_weights=class_weights)
            self.nn = nn
            ...
            if SKLEARN_OK:
                ...
                rf = RandomForestClassifier(
                    ...
                    random_state=42, class_weight=cw_dict
                )
            ...
            self.info = {
                ...
                "train": split, "test": n - split,
                "h1": h1, "h2": h2, "h3": h3,
                "threshold": threshold, "mode": "Entrenamiento ensemble",
                "sklearn": SKLEARN_OK, **test_metrics
            }
            self._save_nn(asset, tf)
            logger.info(f"Entrenamiento completado: {asset}/{tf} | {test_metrics}")
            return True
        except Exception as e:
            self.ready = True
            self.fallback = True
            self.info = {"error": str(e)}
            logger.error(f"Error entrenando {asset}/{tf}: {e}")
            return False
    ...
    def load_or_train(self, asset, tf, df, f):
        ...
        if loaded:
            self.nn = loaded
            self._expected_features = loaded.inp
            self.ready = True
            self.fallback = False
            self.info = {"mode": "Cargado desde nube", "source": "Supabase"}
            logger.info(f"Modelo cargado desde nube: {asset}/{tf}")
            return True
    ...
    def online_update(self, df, f, current_hash, last_hash):
        if not self.ready or self.fallback or self.nn is None or current_hash == last_hash:
            return False
        if self.asset is None or self.tf is None:
            logger.warning("online_update: asset/tf no definidos")
            return False
        try:
            X, y, _ = dataset(df, f)
            if len(X) > 5:
                ...
                self.nn.train(X[-batch:], y[-batch:], epochs=3, batch_size=batch, class_weights=class_weights)
                self.info["mode"] = f"Online learning ({batch})"
                self.info["last_update"] = datetime.now().strftime("%H:%M:%S")
                self._save_nn(self.asset, self.tf)
                logger.info(f"Online update: {self.asset}/{self.tf} ({batch} muestras)")
                return True
        except Exception as e:
            logger.error(f"Error en online_update: {e}")
        return False
    ...
    def predict(self, df, f):
        ...
        try:
            ...
            if self._expected_features is not None and inp.shape[1] != self._expected_features:
                logger.warning(
                    f"Mismatch de features: esperado {self._expected_features}, recibido {inp.shape[1]}. "
                    f"Usando pad/crop."
                )
